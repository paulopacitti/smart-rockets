...
int debug(){
  ...
  printf("____________Testing UpdateRocketPosition function________________\n\n");
  Board *board_2 = newBoard(width, height, target, targetRadius, n_obstacles);
  Population *population_1 = newPopulation(population_size, dna_length, initial_position, mutation_factor);
  printf("Population size: %d \n", population_1->size);
  Rocket *rocket = newRocket(dna_length, initial_position);
  
  printf("Rocket position: (%.2f,%.2f) \n",  rocket->x, rocket->y);
  printf("Rocket status: %d \n",  rocket->live);
  printf("Rocket velocity x: %.2f \n",  rocket->velocity[0]);
  printf("Rocket velocity y: %.2f \n",  rocket->velocity[1]);
  printf("\n");
  ...
  for(int idx = 0; idx<population_1->size; idx++)
  {
    printf("Rocket %d position: (%.2f,%.2f) \n", idx, population_1->rockets[idx]->x, population_1->rockets[idx]->y);
    printf("Rocket %d status: %d \n", idx, population_1->rockets[idx]->live);
    printf("Rocket %d velocity x: %.2f \n", idx, population_1->rockets[idx]->velocity[0]);
    printf("Rocket %d velocity y: %.2f \n", idx, population_1->rockets[idx]->velocity[1]);
    printf("\n");
  } 
